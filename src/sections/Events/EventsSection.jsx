...
const EventsSection = () => {
  return (
    <div className='flex flex-col justify-center items-center px-4 lg:px-16 py-8 lg:py-20 gap-12 relative'>
     <div className='border border-[#D9D9D926] rounded-2xl p-3 flex flex-row justify-between self-start items-center w-[50%]'><input placeholder='Search for an event' className='bg-transparent  placeholder:text-[#6D6767]'/>
     <FontAwesomeIcon icon={faSearch} style={{color:"#6D6767"}}/></div>
     <div className='flex flex-row gap-3 self-start'>{tags.map((tag) =>{
        return(
    ...
    <div className='grid lg:grid-cols-4 gap-8 lg:gap-y-12 bg-[#020302] py-6 lg:py-8'>
      {
        events?.paid?.map((events, index) =>{
          return(
            <Link
                    to={`/event-details/${events.id}`}><EventsCard key={index} events={events}/></Link>
